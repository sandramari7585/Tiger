module.exports = function ({ api, models }) {
    ...
    setInterval(async () => {
      ...
      try {
        if (day != day_now) {
            ...
            console.log('Prince Dilshad');
            await new Promise(async resolve => {
                for (const checkttFile of checkttData) {
                    const checktt = JSON.parse(fs.readFileSync(checkttDataPath + checkttFile));
                    let storage = [], count = 1;
                    for (const item of checktt.day) {
                        const userName = await Users.getNameUser(item.id) || 'Prince Dilshad';
                    ...
                    let checkttBody = '==PRINCE DILSHAD ❤️==\n\n';
            ...
            await new Promise(async resolve => {
                if (day_now == 1) {
                    console.log('Price Dilshad');
                    for (const checkttFile of checkttData) {
                        const checktt = JSON.parse(fs.readFileSync(checkttDataPath + checkttFile));
                        let storage = [], count = 1;
                        for (const item of checktt.week) {
                            const userName = await Users.getNameUser(item.id) || 'Price Hun Yar';
                            const itemToPush = item;
                            itemToPush.name = userName;
                            storage.push(itemToPush);
                        };
    ...
    (async function () {
        ...
        try {
            logger(global.getText('listen', 'startLoadEnvironment'), '[ Price Dilshad ]');
    ...
    logger(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, "[ Prince Dilshad]");
