...
module.exports.onLoad = async() => {
    ...
    const path = resolve(__dirname, 'cache/canvas', '143.jpg');
    if (!existsSync(dirMaterial + "canvas")) mkdirSync(dirMaterial, { recursive: true });
    if (!existsSync(path)) await downloadFile("https://imgur.com/gallery/2g7qGlV.png", path);
}
...
async function makeImage({ one, two }) {
    ...
    let tromcho_img = await jimp.read(__root + "/143.jpg");
